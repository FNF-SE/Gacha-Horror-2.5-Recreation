...
function createTrailFrame(tag)
  ...
  if tag == 'BF' then
    ...
    if trailEnabledBF then
      ...
      local imageFile = getProperty('boyfriend.imageFile')
      local animName = getProperty('boyfriend.animation.curAnim.name')
      local isMultiAtlas = getProperty('boyfriend.isMultiAtlas')
    ...
    if trailEnabledDad then
      ...
      local imageFile = getProperty('dad.imageFile')
      local animName = getProperty('dad.animation.curAnim.name')
      local isMultiAtlas = getProperty('dad.isMultiAtlas')
      local newPath = isMultiAtlas and (imageFile:match("(.+)/[^/]+$") or imageFile)..'/'..animName or imageFile
      image = newPath
