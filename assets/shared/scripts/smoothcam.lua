...
function opponentNoteHit(id, dir)
  if getProperty('characterPlayingAsDad') == false then shiftCamNote(dadShift, dir) else shiftCamNote(bfShift, dir) end
end

function goodNoteHit(id, dir, typ, sus)
  if getProperty('characterPlayingAsDad') == false then shiftCamNote(bfShift, dir) else shiftCamNote(dadShift, dir) end
end
...
function noteMissPress(key)
  if getProperty('characterPlayingAsDad') == false then shiftCamNote(bfShift, key) else shiftCamNote(dadShift, key) end
end
...
function onStepHit()
  ...
  if (getProperty("camZooming") and getCamProperty("zoom") < 1.35 and getPropertyFromClass("backend.ClientPrefs", "data.camZooms")) then
    camZoom = camZoom + .015
  end
end
