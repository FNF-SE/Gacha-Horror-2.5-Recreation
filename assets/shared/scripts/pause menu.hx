...
var options:Array<String> = ['Resume', 'Restart', 'Chart Editor', 'Options', 'Exit'];
...
function onCustomSubstateCreate(t)
{
	switch (t)
	{
		case 'pauseMenu':
			...
			{
				var option = new FlxText(0, ((i + 1) * 80) + bithc, 0, options[i], 40);
			...
			curSelect = new FlxText(((optionObjects.get('option' + curSelected).x + optionObjects.get('option' + curSelected).width) + 10),
				optionObjects.get('option' + curSelected).y, 0, '<', 40);
			...
			songTxt.font = Paths.font('vcr.ttf');
			...
			diff = new FlxText(songTxt.x + 200, songTxt.y + 40, 300, 'Difficulty: ' + "HARD", 30);
			diff.font = Paths.font('vcr.ttf');
			...
			deathCount.font = Paths.font('vcr.ttf');
			...
			if (settings.music != 'Song Inst')
				pauseMusic.loadEmbedded(Paths.music(convertPauseMenuSong(settings.music)), true);
			else
				pauseMusic.loadEmbedded(Paths.inst(convertPauseMenuSong(PlayState.SONG.song)), true);
...
function onCustomSubstateUpdate(t, elapsed)
{
	switch (t)
	{
		case 'pauseMenu':
			...
			{
				if (Controls.instance.UI_UP_P || Controls.instance.UI_DOWN_P)
					changeSelection(Controls.instance.UI_UP_P ? -1 : 1);

				for (i in 0...options.length)
				{
					overlappingOption = mouseOverlaps(optionObjects.get('option' + i));
					if (!overlappingOption && optionCooldown >= 0)
						optionCooldown -= elapsed;
				...
				if (game.controls.ACCEPT || (mouseOverlaps(optionObjects.get('option' + curSelected)) && FlxG.mouse.justPressed))
				{
			...
			if (pauseMusic.volume < 0.5 && ClientPrefs.data.pauseMusic != 'None')
...
function changeSelection(?dir:Int)
{
	...
	FlxTween.tween(curSelect, {x: (optionObjects.get('option' + curSelected).x + optionObjects.get('option' + curSelected).width) + 10},
		settings.optionTweenTime, {ease: FlxEase.quadOut});

	FlxTween.tween(curSelect, {y: optionObjects.get('option' + curSelected).y}, settings.optionTweenTime, {ease: FlxEase.quadOut});
...
function fadeIn()
{
	...
	for (i in 0...options.length)
		FlxTween.tween(optionObjects.get('option' + i), {alpha: 1, x: 60}, settings.openMenuTweenTime + (i / 20), {
...
{
	if (state != 'options')
		FlxTween.tween(pauseMusic, {volume: 0}, 0.25, {ease: FlxEase.quadOut});
	...
	if (state == null)
	{
		...
		for (i in 0...options.length)
			FlxTween.tween(optionObjects.get('option' + i), {alpha: 0}, 0.25, {ease: FlxEase.quadOut});
		...
		FlxTween.tween(bgGrid, {alpha: 0}, 0.25, {
			ease: FlxEase.quadOut,
			onComplete: function()
			{
				if (!game.mobileControls.instance.visible)
					game.mobileControls.instance.visible = true;
				if (game.touchPad != null && !game.touchPad.visible)
					game.touchPad.visible = true;
				game.canPause = true;
	...
	{
		FlxTween.tween(fadeOutSpr, {alpha: 1}, 0.25, {
			ease: FlxEase.quadOut,
			onComplete: function()
			{
				...
				switch (state)
				{
					...
					case 'options':
						...
						if (ClientPrefs.data.pauseMusic != 'None')
						{
							...
							FlxTween.tween(pauseMusic, {volume: 0}, 0.2);
...
function mouseOverlaps(object, ?offsetX:Float, ?offsetY:Float)
{
	...
	var overlapX:Bool = (FlxG.mouse.getScreenPosition(game.camOther)
		.x + offsetX) >= object.x && (FlxG.mouse.getScreenPosition(game.camOther).x + offsetX) <= object.x + object.width;
	var overlapY:Bool = (FlxG.mouse.getScreenPosition(game.camOther)
		.y + offsetY) >= object.y && (FlxG.mouse.getScreenPosition(game.camOther).y + offsetY) <= object.y + object.height;
