...
class PlayState extends MusicBeatState
{
	...
	public var sarahGroup:FlxSpriteGroup;
	public var sarahCameraOffset:Array<Float> = null;
	public var SARAH_X:Float = 1080;
	public var SARAH_Y:Float = 0;

	public var hud:HUDImpl;

	public var flashRed:FlxSprite;
	...
	override public function create()
	{
		...
		sarahCameraOffset = [0, 0];

		boyfriendGroup = new FlxSpriteGroup(BF_X, BF_Y);
		dadGroup = new FlxSpriteGroup(DAD_X, DAD_Y);
		gfGroup = new FlxSpriteGroup(GF_X, GF_Y);
		sarahGroup = new FlxSpriteGroup(SARAH_X, SARAH_Y);

		switch (curStage)
		{
			case 'Gachastage' | 'stage':
				new states.stages.GachaStage();
			case 'Gacha Hell':
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'scripts/'))
		{
			for (file in Paths.readDirectory(folder))
			{
				...
				if (file.toLowerCase().endsWith('.hx'))
					initHScript(#if MODS_ALLOWED folder + file #else Assets.getText(folder + file) #end);
				#end
			}
		}
		#end
		...
		sarah = new Character(0, 0, "sarahghost");
		sarahGroup.add(sarah);
		sarahGroup.visible = false;
		...
		if (SONG.song != "Betrayal")
			sarahGroup.remove(sarah);
		else
			Paths.image('characters/SarahGhost', 'shared');
	...
	override public function update(elapsed:Float)
	{
		...
		if ((controls.PAUSE #if android || FlxG.android.justReleased.BACK #else || touchPad.buttonP.justPressed #end)
	...
	{
		if (SONG.song == "Sentient" || SONG.song == "Crisis")
	...
	public function endSong()
	{
		...
		inCutscene = false;
		updateTime = false;
		inSilhouette = characterPlayingAsDad = false;
