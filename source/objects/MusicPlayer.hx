...
/**
 * Music player used for Psych Freeplay
 */
	...
	override function update(elapsed:Float)
	{
		...
		if (instance.controls.UI_LEFT_P)
		{
			...
			if (FreeplayStateOld.vocals != null)
				FreeplayStateOld.vocals.time = curTime;
		}
		...
		if (FreeplayStateOld.vocals != null && FlxG.sound.music.time > 5)
		{
			var difference:Float = Math.abs(FlxG.sound.music.time - FreeplayStateOld.vocals.time);
			if (difference >= 5 && !paused)
			{
				pauseOrResume();
				FreeplayStateOld.vocals.time = FlxG.sound.music.time;
				pauseOrResume(true);
			}
		}
		updatePlaybackTxt();
		...
		if (instance.touchPad.buttonC.justPressed || instance.controls.RESET)
		{
			...
			if (FreeplayStateOld.vocals != null)
				FreeplayStateOld.vocals.time = 0;
